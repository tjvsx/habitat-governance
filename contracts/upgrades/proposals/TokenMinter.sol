...
the diamond is callable in a specific way (no function collisions allowed), but the diamond can call other contracts in an arbitrary way, so function selectors can be preset in a call within the diamond's facet (ex: governance.execute()) and calling it can lead to many different functionalities. 
