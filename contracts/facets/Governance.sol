...
contract Governance {
    ...
    enum ProposalStatus { 
        NoProposal,                     // 0
        PassedAndReadyForExecution,     // 1
        RejectedAndReadyForExecution,   // 2
        PassedAndExecutionStuck,        // 3
        VotePending,                    // 4
        Passed,                         // 5
        Rejected                        // 6
    }
    ...
    function unvote(uint256 _proposalId) external {
        ...
        GovernanceStorage.Proposal storage p = gs.proposals[_proposalId];
        require(block.timestamp < p.deadline, 'Governance: Voting ended'); 
        require(p.proposer != msg.sender, 'Governance: Can\'t unvote your own proposal');       
